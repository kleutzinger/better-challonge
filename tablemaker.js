...
function genTable(plist){
    ...
    for (const [idx, p] of pArray.entries()){
        if (lastPlacing != p.placing){
            firstlines = `<tr><td class="rank" rowspan="${playersPerPlacing[p.placing]}"><span class=${"rank"+p.placing}>${p.placing}</spa</td>`}
        else {firstlines = "<tr>";}
        winSpan = genWLSpans(p,"w");
        lossSpan = genWLSpans(p,"l");
        entryTemplate =`
            <td class="left display_name">
                <span><strong><span class="${p.id +"class"}" title="Seeded: ${p.seed}">${p.name}</strong></span>
            </td>
